...
export default function HomePage() {
  ...
  const getCurrentTimestamp = async () => {
    if (atm) {
      try {
        const timestampBigNumber = await atm.getCurrentTimestamp();
        setTimestamp(timestampBigNumber.toString());
      } catch (error) {
  ...
  const lockTokens = async () => {
    if (atm) {
      try {
        const amountInWei = ethers.utils.parseUnits(lockAmount, 18); // Assuming the token has 18 decimals
        const tx = await atm.lockTokens(amountInWei);
        await tx.wait();
        alert(`Locked ${lockAmount} tokens`);
        getBalance(); // Update balance after successful locking
      } catch (error) {
  ...
  const unlockTokens = async () => {
    if (atm) {
      try {
        ...
        getBalance(); // Update balance after successful unlocking
      } catch (error) {
